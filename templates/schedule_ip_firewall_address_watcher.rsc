...
:if ([:len $intfIPList] = 0) do={
    :log error "RSPM Watcher: no ip found on $intfList!";
...
:foreach v in $intfIPList do={
    ...
    :if ([:len $idList] > 0) do={
        ...
        :log info "RSPM Watcher: current ip $v not found in address list $bindedAddressList.";
    }
}

# compare
if ($cnt < [:len $intfIPList]) do={
    :log info "RSPM Watcher: updating address list $bindedAddressList ..."
    [/ip/firewall/address-list/remove [/ip/firewall/address-list/find list=$bindedAddressList]];
